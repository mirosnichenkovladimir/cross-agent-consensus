...
10. If an Author/Reviewer/Validator participant is an explicitly named and authorized external agent CLI or host tool, run `scripts/consensus invocation-ready` when present, then invoke it with `scripts/consensus invoke-agent` after saving the prompt; do not stop at a manual handoff merely because it is cross-runtime. Do not replace a named Codex/Claude/Hermes reviewer with a host-internal subagent or in-chat review when the CLI can be run, because that bypasses `rounds/<round>/agents/<actor>/session-*` telemetry. When ConfigResolution records `reviewer_clis.<reviewer>.command`, terminal validation requires a completed reviewer invocation session for RawReviewerOutput from that reviewer. Authorization means the operator has approved the exact command for this run, or Policy declares `unattended_invocation: true` with scope limits. Persistent config files must not enable unattended invocation. Store raw stdout/stderr or raw model output under `rounds/round-NNN/raw/` and/or the appropriate lifecycle record with `scripts/consensus capture` when present before normalization. A CLI is available when its binary is executable and its output can be captured to the run folder; authentication failures, timeouts, and non-zero exits are runtime errors to record, not availability failures.
...
Reviewer CLI mappings are explicit command presets only. They do not select providers dynamically, do not bypass `invocation-ready`, and do not make a cross-runtime runner automatic.
